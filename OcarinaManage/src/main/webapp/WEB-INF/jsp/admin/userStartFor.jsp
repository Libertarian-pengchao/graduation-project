...
<body>
	<div class="easyui-layout" data-options="fit:true">
		...
		<div id="usersToolbar">
			<div class="usersToolbar_button" style="height: 40px;">
	            <a href="#" class="easyui-linkbutton" iconCls="icon-edit" onclick="user_openForbidden()" plain="true">禁用</a>
	            <a href="#" class="easyui-linkbutton" iconCls="icon-edit" onclick="user_openStart()" plain="true">启用</a>
	...
	<script type="text/javascript">
		...
		function user_openForbidden(){
			var rows = $("#usersDatagrid").datagrid("getSelections");
			if(rows.length < 1){
				$.messager.alert("信息提示","请选择您要禁用的用户！","info");
			}else{
				var ids = new Array();
				...
				$.ajax({
					url:"/OcarinaManage/userForbidden",
					method:"post",
					data:{
						ids:ids
					},
		...
		function user_openStart(){
			var rows = $("#usersDatagrid").datagrid("getSelections");
			if(rows.length < 1){
				$.messager.alert("信息提示","请选择您要启用的用户！","info");
			}else{
				var ids = new Array();
				for(var i=0; i < rows.length; i++){
					ids.push(rows[i].id);
