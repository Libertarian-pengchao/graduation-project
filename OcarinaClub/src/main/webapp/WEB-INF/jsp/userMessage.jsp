...
<body>
	<div class="container-fluid">
		...
		<section class="panel">
			...
			<h3>${user.userName}</h3>
			<p class="leadsex">${user.sex}</p>
			<p class="lead">${user.personalsign}</p>
			<p>
				<a class="btn" href="<%=response.encodeURL(request.getContextPath() + "/myVideos")%>"> 
					<i class="fa fa-bars"></i> 我的作品
				</a>
				 <a class="btn"
					href="<%=response.encodeURL(request.getContextPath() + "/updateUserMessage")%>">
					<i class="fa fa-bars"></i> 编辑资料
				</a>
				<a class="btn" href="<%=response.encodeURL(request.getContextPath() + "/redUpdateUserImg")%>"> 
					<i class="fa fa-bars"></i> 修改头像
				</a>
			</p>
			<p class="tags">
